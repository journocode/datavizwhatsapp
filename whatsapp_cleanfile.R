...
chat <- read.csv("[insert file path]",
                 header = FALSE, sep = ";",
...
#Add 4 empty rows to the end to make space for shifting
chat <- cbind(chat, matrix(nrow = nrow(chat), ncol = 5))

#Shift stampless rows 4 cols to the left, fill with NAs
...
#Convert columns 1 and 2 into date format:

#Merge columns 1 and 2 to simplify things
